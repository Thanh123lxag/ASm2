...
function showCart() {
        ...
        if (currentuser.cart.length != 0) {
            ...
            currentuser.cart.forEach(item => {
                ...
                <div class="cart-item-control">
                    <button class="cart-item-delete" onclick="deleteCartItem(${product.id},this)"><i class="fa-solid fa-trash-can"></i> Xóa</button>
                    <div class="buttons_added">
                        <input class="minus is-form" type="button" value="-" onclick="decreasingNumber(this)">
    ...
    let themmon = document.querySelector('.them-sp');
...
function kiemtradangnhap() {
    ...
    if (currentUser != null) {
        ...
        document.querySelector('.auth-container').innerHTML = `<span class="text-lisu">Tài khoản</span>
            <span class="text-acc">${user.fullname} <i class="fa-sharp fa-solid fa-caret-down"></span>`
        document.querySelector('.header-middle-right-menu').innerHTML = `<li><a href="javascript:;" onclick="myAccount()"><i class="fa-regular fa-circle-user"></i> Tài khoản của tôi</a></li>
...
function checkAdmin() {
    ...
    if(user && user.userType == 1) {
        ...
        node.innerHTML = `<a href="./index3.html"><i class="fa-solid fa-people-roof"></i> Quản lý cửa hàng</a>`
...
function renderOrderProduct() {
    ...
    if (arrDonHang.length == 0) {
        ...
        // Duyệt qua từng đơn hàng và hiển thị chi tiết
        arrDonHang.forEach(item => {
            ...
            let iconCompl = item.trangthai == 1 ? "fa-check-circle" : "fa-hourglass-start"; // Thay biểu tượng cho trạng thái
            let textCompl = item.trangthai == 1 ? "Đã xử lý" : "Đang xử lý"; // Văn bản tương ứng

            productHtml += `<div class="order-history-control">
                <div class="order-history-status">
                    <i class="fa-solid ${iconCompl}"></i> ${textCompl} 
    ...
    // Hiển thị tất cả đơn hàng vào phần tử DOM
...
function renderProducts(showProduct) {
    ...
    if(showProduct.length == 0) {
        ...
        productHtml = `<div class="no-result"><div class="no-result-h">Tìm kiếm không có kết quả</div><div class="no-result-p">Xin lỗi, chúng tôi không thể tìm được kết quả hợp với tìm kiếm của bạn</div><div class="no-result-i"><i class="fa-solid fa-ban"></i></div></div>`;
    } else {
